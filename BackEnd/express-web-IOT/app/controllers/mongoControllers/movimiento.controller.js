const model = require('../../models/mongoModels/Movimiento_vehiculo');
...
const listar_movimiento_id = async(req,res) => {
  let {id} = req.params;
  try {
    const item = await model.findOne({ id_movimiento: id });
    if(!item){
...
const upd_movimiento = async(req,res) => {
  let {id} = req.params;
  try {
    const actualizado = await model.findOneAndUpdate({ id_movimiento: id }, req.body, { new: true });
    if(!actualizado){
...
let {id} = req.params;
  try {
    const borrado = await model.findOneAndDelete({ id_movimiento: id });
        if(!borrado){
      return res.status(404).json({msg:"No hay un movimiento con ese id"});
    }
